# Kentucky Residential Electricity Sales
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//ELEC.SALES.KY-RES.M?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('Kentucky_Residential_Electricity_Sales.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to Kentucky_Residential_Electricity_Sales.csv")
else:
