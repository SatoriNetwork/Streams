# Natural Gas Futures Contract 4
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//NG.RNGC4.D?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('Natural_Gas_Futures_Contract_4.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to Natural_Gas_Futures_Contract_4.csv")
else:
