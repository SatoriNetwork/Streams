# Tennessee Nuclear Energy Consumption
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//SEDS.NUETB.TN.A?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('Tennessee_Nuclear_Energy_Consumption.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to Tennessee_Nuclear_Energy_Consumption.csv")
else:
