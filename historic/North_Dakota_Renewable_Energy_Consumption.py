# North Dakota Renewable Energy Consumption
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//SEDS.RETCB.ND.A?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('North_Dakota_Renewable_Energy_Consumption.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to North_Dakota_Renewable_Energy_Consumption.csv")
else:
