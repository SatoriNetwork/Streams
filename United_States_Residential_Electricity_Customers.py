# United States Residential Electricity Customers
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//ELEC.CUSTOMERS.US-RES.M?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('United_States_Residential_Electricity_Customers.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to United_States_Residential_Electricity_Customers.csv")
else:
