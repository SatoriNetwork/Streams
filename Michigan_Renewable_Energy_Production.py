# Michigan Renewable Energy Production
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//SEDS.REPRB.MI.A?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('Michigan_Renewable_Energy_Production.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to Michigan_Renewable_Energy_Production.csv")
else:
