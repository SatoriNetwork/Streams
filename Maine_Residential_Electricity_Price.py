# Maine Residential Electricity Price
# Generate CSV
import requests
import json
...
url = "https://api.eia.gov//v2//seriesid//ELEC.PRICE.ME-RES.M?api_key=wxFRLAoaTMQ9Ra7NvakhNKSxxstutZsG28nuerWR"
...
if response.status_code == 200:
    ...
    with open('Maine_Residential_Electricity_Price.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['index', 'value'])  # Write the header
        csvwriter.writerows(result)

    print("Data has been saved to Maine_Residential_Electricity_Price.csv")
else:
