# ZHVI All Homes (SFR, Condo/Co-op)		
# Generate CSV
import requests
import json
...
url = "https://data.nasdaq.com/api/v3/datatables/ZILLOW/DATA?indicator_id=ZALL&api_key=zjsgHD-jbXKBVezYzyqT"
...
if response.status_code == 200:
    ...
    sorted_data = sorted(time_series, key=lambda x: dt.datetime.strptime(x[2], '%Y-%m-%d'))
    # Open a CSV file for writing
    with open('ZHVI_All_Homes_(SFR, Condo_or_Co-op).csv', 'w', newline='') as csvfile:
        ...
        for items in sorted_data:
            formatted_date = dt.datetime.strptime(items[2] + ' 00:00:00.000000', '%Y-%m-%d %H:%M:%S.%f')
    ...
    print("Data has been saved to ZHVI_All_Homes_(SFR, Condo_or_Co-op).csv")
else:
